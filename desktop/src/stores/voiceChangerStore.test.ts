...
describe('voiceChangerStore', () => {
  ...
  it('读取并保存保护值参数', async () => {
    ...
    await useVoiceChangerStore.getState().loadParameters(client)
    await useVoiceChangerStore.getState().saveProtect(0.27, client)

    expect(useVoiceChangerStore.getState().protect).toBe(0.27)
    expect(useVoiceChangerStore.getState().parametersError).toBeNull()
    expect(savedParameters).toEqual([
      {
        ...defaultParameters,
  ...
  it('读取并保存输入阈值参数', async () => {
    ...
    await useVoiceChangerStore.getState().loadParameters(client)
    await useVoiceChangerStore.getState().saveInputThresholdDb(-52, client)

    expect(useVoiceChangerStore.getState().inputThresholdDb).toBe(-52)
    expect(useVoiceChangerStore.getState().parametersError).toBeNull()
    expect(savedParameters).toEqual([
      {
        ...defaultParameters,
  ...
  it('读取并保存输出增益参数', async () => {
    ...
    await useVoiceChangerStore.getState().loadParameters(client)
    await useVoiceChangerStore.getState().saveOutputGainDb(6, client)

    expect(useVoiceChangerStore.getState().outputGainDb).toBe(6)
    expect(useVoiceChangerStore.getState().parametersError).toBeNull()
    expect(savedParameters).toEqual([
      {
        ...defaultParameters,
  ...
  it('读取并保存降噪开关参数', async () => {
    ...
    await useVoiceChangerStore.getState().loadParameters(client)
    await useVoiceChangerStore.getState().saveDenoise(false, client)

    expect(useVoiceChangerStore.getState().denoise).toBe(false)
    expect(useVoiceChangerStore.getState().parametersError).toBeNull()
    expect(savedParameters).toEqual([
      {
        ...defaultParameters,
